...
plt.figure(1, figsize=(5 * len(families), 5))

# Loop on templates
for i in range(0, len(families)):

    # Open LFE catalog (FAME data)
    namedir = 'catalogs/' + templates[families[i]][0].astype(str)
    ...
    # Filter LFEs
    df_FAME = df_FAME.loc[df_FAME['cc'] * df_FAME['nchannel'] >= threshold['threshold_FAME'].iloc[families[i]]]
    df_perm = df_perm.loc[df_perm['cc'] * df_perm['nchannel'] >= threshold['threshold_perm'].iloc[families[i]]]
    ...
    # Plot
    plt.subplot2grid((1, len(families)), (0, i))
