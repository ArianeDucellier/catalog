...
for i in [12, 13, 20, 23, 25]: #range(0, np.shape(templates)[0]):
    ...
    if threshold['threshold_perm'].iloc[i] > 0.0:
        ...
        if (latitude >= 40.2) and (latitude <= 40.8):
            ...
            Xpos = X[X >= 1]
            if len(Xpos) > 0:
                plt.stem(days, longitude + amp * Xpos, 'k-', markerfmt=' ', basefmt=' ', bottom=longitude)
            plt.axhline(longitude, linewidth=1, color='k', linestyle='--')
...
plt.savefig('LFEdistribution_perm/longitude.eps', format='eps')
plt.close(1)
